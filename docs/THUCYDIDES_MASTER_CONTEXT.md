...
Discovery source candidates are not jobs. Broad Glassdoor/listing/search pages remain non-importable. `/discovery` guides Adel in Hebrew RTL through provider testing, enumeration, verified posting review, safe manual import, and cleanup. Phase 6.2 keeps the Hebrew-first RTL foundation and now makes the source review board more action-oriented: current sources with real next actions appear first, generic Workday boards without Israel/remote evidence are demoted, repeated source candidates collapse by canonical display key, already-processed sources move to a secondary section, and low-quality/skipped/unsupported records stay out of the primary work list. Verified postings can be ready, needs review, duplicate, imported, or blocked; duplicate and low-confidence verified postings stay in the verified section instead of legacy/noisy, and ready postings sort first while blocked postings sort last. Cards and core pages clip or wrap noisy provider text, URLs, pasted descriptions, source text, and notes so long content does not create horizontal overflow. Old noisy leads can be hidden by marking them `SKIPPED`, without deletion or touching imported jobs. A verified posting can still be blocked by role rules. A `JobDiscoveryLead` becomes a normal `Job` only after Adel explicitly imports it, and forbidden or non-importable leads are blocked from normal import.
