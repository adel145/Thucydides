...
Discovery source candidates are not jobs. Broad Glassdoor/listing/search pages remain non-importable. `/discovery` guides Adel in Hebrew RTL through provider testing, enumeration, verified posting review, safe manual import, and cleanup. Phase 6.1G extends the Hebrew-first RTL foundation into the shared shell and core workflow pages: Dashboard, Profile, Job Inbox, Job detail/edit/delete, Application Packet, Resume Lab, Agent Council, Pipeline, Sources, Source Detail, Gmail, and Settings. Verified postings can be ready, blocked, duplicate, imported, or need review; duplicate and low-confidence verified postings stay in the verified section instead of legacy/noisy. Cards and core pages clip or wrap noisy provider text, URLs, pasted descriptions, source text, and notes so long content does not create horizontal overflow. Old noisy leads can be hidden by marking them `SKIPPED`, without deletion or touching imported jobs. A verified posting can still be blocked by role rules. A `JobDiscoveryLead` becomes a normal `Job` only after Adel explicitly imports it, and forbidden or non-importable leads are blocked from normal import.
